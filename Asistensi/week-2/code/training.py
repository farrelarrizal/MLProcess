...
# path to the dataset
data_path = os.path.join(config["data_source"]["directory"], config["data_source"]["filename"])
...
model = KNeighborsClassifier(
                            ...
                            leaf_size=config["model"]["leaf_size"],
                            p=config["model"]["p"],
...
joblib.dump(model, os.path.join(config["model"]["model_directory"], "{}".format(model_name)))
